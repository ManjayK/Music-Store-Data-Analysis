...
that has highest sum of invoice totals. Return both the city name and sum off all invoice 
totals. */
SELECT billing_city, SUM(total) AS highest_invoice FROM invoice
...
Write a query that returns the country along with the top customer and how much they spent. For countries where the 
